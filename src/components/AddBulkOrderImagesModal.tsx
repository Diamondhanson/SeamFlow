...
interface AddBulkOrderImagesModalProps {
  visible: boolean;
  onClose: () => void;
  orderId: string;
  orderName: string;
...
const uploadBulkOrderImage = async (imageUri: string, user: any): Promise<string> => {
  try {
    ...
    const fileName = `${user.id}/bulk_orders/${Date.now()}.${fileExt}`;
...
const AddBulkOrderImagesModal: React.FC<AddBulkOrderImagesModalProps> = ({
  visible,
  onClose,
  orderId,
  orderName,
}) => {
  const { updateBulkOrderImages } = useClients();
  const { user } = useApp();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (orderImages.image1Uri && user) {
        uploadedImage1 = await uploadBulkOrderImage(orderImages.image1Uri, user);
      }
      if (orderImages.image2Uri && user) {
        uploadedImage2 = await uploadBulkOrderImage(orderImages.image2Uri, user);
      }

      // Update the bulk order with image URLs
      await updateBulkOrderImages(orderId, uploadedImage1, uploadedImage2);
      ...
      Alert.alert('Success', 'Images added to bulk order successfully!');
    } catch (error) {
      console.error('Error adding images to bulk order:', error);
    ...
    >
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          ...
          <Text style={styles.orderName}>Bulk Order: {orderName}</Text>
          <Text style={styles.description}>
            Add up to 2 images to help visualize this bulk order
          </Text>
...
export default AddBulkOrderImagesModal; 