...
export default function Thread() {
  ...
  const promptAttach = async () => {
    const action = await dialog.choose<'camera' | 'library'>({
      title: t('cchat.attach'),
      actions: [
        { label: t('cchat.attachTakePhoto'), value: 'camera' },
        { label: t('cchat.attachFromGallery'), value: 'library' },
      ],
    });
  ...
  const onFailedPress = async (p: PendingMessage) => {
    const action = await dialog.choose<'retry' | 'discard'>({
      title: t('cchat.failedToSend'),
      actions: [
        { label: t('cchat.retry'), value: 'retry' },
  ...
  const renderRow = ({ item }: { item: Row }) => {
    ...
    const mine =
      item.kind === 'pending' ? true : item.msg.senderType === 'client';
        ...
        >
          ...
          <View style={styles.meta}>
            {item.kind === 'pending' ? (
              ...
              >
                {failed
                  ? `${t('cchat.failedToSend')} · ${t('cchat.tapToRetry')}`
                  : online
                    ? t('cchat.sending')
                    : t('cchat.queuedOffline')}
              </Text>
            ) : mine ? (
              <Text variant="caption" style={{ color: atelier.textOnPrimary }}>
                {item.msg.readAt ? t('cchat.readReceipt') : t('cchat.deliveredReceipt')}
              </Text>
            ) : null}
          </View>
        </View>
  ...
  return (
    <Screen padded={false}>
      <View style={styles.padded}>
        <ScreenHeader
          title={conversation?.counterparty.name ?? t('cchat.threadTitle')}
          right={
            conversation?.orderId ? (
              <Pressable
                onPress={() =>
                  router.push({
                    pathname: '/hub/orders/[id]',
      ...
      >
        ...
        <TextInput
          ...
          placeholder={t('cchat.composerPlaceholder')}
        ...
        <Pressable
          onPress={send}
          disabled={!draft.trim()}
          accessibilityLabel={t('cchat.send')}
          style={[
            styles.sendBtn,
            {
