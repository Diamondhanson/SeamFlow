...
export default function RequestDetail() {
  ...
  const onAccept = async (offerId: string) => {
    const ok = await dialog.confirm({
      title: t('crequests.acceptTitle'),
      message: t('crequests.acceptBody'),
      confirmLabel: t('crequests.acceptConfirm'),
    });
    if (!ok) return;
    accept.mutate(offerId, {
      onSuccess: (res) => router.replace(`/hub/messages/${res.conversationId}`),
  ...
  const onClose = async () => {
    const ok = await dialog.confirm({
      title: t('crequests.closeTitle'),
      message: t('crequests.closeBody'),
      confirmLabel: t('crequests.closeConfirm'),
      destructive: true,
    });
  ...
  if (q.isLoading) {
    return (
      <Screen>
        <ScreenHeader title={t('crequests.detailTitle')} />
        <SkeletonDetail />
      </Screen>
    );
  }
  if (!request) {
    return (
      <Screen>
        <ScreenHeader title={t('crequests.detailTitle')} />
        <FormScroll>
          <Text variant="bodySm" tone="textMuted">{t('crequests.gone')}</Text>
        </FormScroll>
      </Screen>
    );
  }
    ...
    <Screen>
      ...
      <FormScroll contentContainerStyle={{ paddingBottom: 96 }}>
        ...
        <Card>
          ...
          <CardLine>{t('crequests.statusLine', { status: request.status })}</CardLine>
        </Card>

        <Text variant="h3" style={{ marginTop: spacing.lg }}>
          {t('crequests.offersHeading', { count: offers.length })}
        </Text>
          ...
          offers.map((o) => (
            <Card key={o.id}>
              ...
              <CardLine>
                {o.price
                  ? o.priceMax
                    ? t('crequests.offerRange', {
                        min: o.price,
                        max: o.priceMax,
                        currency: o.currency ?? '',
                      })
                    : t('crequests.offerPrice', { price: o.price, currency: o.currency ?? '' })
                  : t('crequests.offerToDiscuss')}
              </CardLine>
              ...
              {open && o.status !== 'declined' && o.status !== 'withdrawn' ? (
                <>
                  ...
                  <Button
                    label={t('crequests.chooseThisTailor')}
                    onPress={() => onAccept(o.id)}
                    loading={accept.isPending}
                  />
                </>
              ) : (
                <CardLine>{t('crequests.offerStatus', { status: o.status })}</CardLine>
              )}
            </Card>
          ))
        )}
        ...
        {open ? (
          <>
            ...
            <Button
              label={t('crequests.closeRequest')}
              variant="ghost"
              size="sm"
              onPress={onClose}
            />
