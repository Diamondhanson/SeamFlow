...
import { useCreateRequest } from '../../../../lib/consumer-queries';
...
export default function NewRequest() {
  ...
  const chooseSource = async () => {
    const pick = await dialog.choose<'camera' | 'library'>({
      title: t('crequests.addPhotoTitle'),
      actions: [
        { label: t('crequests.takePhoto'), value: 'camera' },
        { label: t('crequests.chooseFromGallery'), value: 'library' },
      ],
    });
  ...
  const post = () => {
    ...
    create.mutate(
      ...
      {
        onSuccess: (created) => {
          router.replace(`/hub/requests/${created.id}`);
        },
  ...
  return (
    <Screen>
      <ScreenHeader title={t('crequests.newTitle')} />
      <FormScroll contentContainerStyle={{ paddingBottom: 120 }}>
        <Text variant="bodySm" tone="textMuted">{t('crequests.newIntro')}</Text>
          ...
          {previews.map((uri, i) => (
            <View key={uri} style={styles.thumbWrap}>
              ...
              <Pressable
                ...
                accessibilityLabel={t('crequests.removePhoto')}
              >
        ...
        <Input
          label={t('crequests.descriptionLabel')}
          placeholder={t('crequests.descriptionPlaceholder')}
          value={description}
