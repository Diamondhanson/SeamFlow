...
import { useMyRequests } from '../../../../lib/consumer-queries';
        ...
        {q.isLoading ? (
          ...
          <View style={styles.empty}>
            ...
            <Text variant="bodySm" tone="textMuted" style={styles.emptyText}>
              {t('crequests.mineEmpty')}
            </Text>
          </View>
        ) : (
          items.map((r) => (
            <Pressable key={r.id} onPress={() => router.push(`/hub/requests/${r.id}`)}>
              <Card>
                <CardTitle>{r.title || garmentLabel(r.garmentType, language)}</CardTitle>
                <CardLine>{t('crequests.statusLine', { status: r.status })}</CardLine>
                <CardLine>{t('crequests.offerCount', { count: r.offersCount })}</CardLine>
              </Card>
            </Pressable>
          ))
        )}
