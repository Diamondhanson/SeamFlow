...
export default function DeleteAccount() {
  ...
  const reauthenticate = async () => {
    ...
    throw new Error(t('caccount.reauthUnavailable'));
  };
    ...
    try {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(file.uri, {
          ...
          dialogTitle: t('caccount.exportTitle'),
        });
      } else {
        await dialog.alert({
          title: t('caccount.exportSavedTitle'),
          message: t('caccount.exportSavedBody'),
          tone: 'success',
        });
      }
    } catch (err) {
  ...
  const onDelete = async () => {
    const sure = await dialog.confirm({
      title: t('caccount.confirmTitle'),
      message: t('caccount.confirmBody'),
      confirmLabel: t('caccount.confirmAction'),
      tone: 'error',
    });
    if (!sure) return;
    ...
    try {
      ...
      await refetch();
      await dialog.alert({
        title: t('caccount.scheduledTitle'),
        message: t('caccount.scheduledBody', { days: state.daysRemaining ?? 30 }),
        tone: 'success',
      });
      router.replace('/(app)');
    } catch (err) {
  ...
  return (
    <Screen>
      <ScreenHeader title={t('caccount.deleteTitle')} />
      <ScrollView
      ...
      >
        ...
        >
          ...
          <Text variant="bodySm" tone="text" style={styles.noticeText}>
            {t('caccount.warningBody', { days: 30 })}
          </Text>
        </View>

        <Text variant="h3" style={styles.heading}>{t('caccount.whatGoesTitle')}</Text>
        {[
          t('caccount.goesRequests'),
          t('caccount.goesMessages'),
          t('caccount.goesMeasurements'),
          t('caccount.goesAccount'),
        ].map((line) => (
        ...
        <Text variant="h3" style={styles.heading}>{t('caccount.takeItTitle')}</Text>
        <Text variant="bodySm" tone="textMuted" style={styles.para}>
          {t('caccount.takeItBody', { days: 30 })}
        </Text>
        <Button
          label={t('caccount.exportAction')}
          variant="secondary"
          loading={exporting}
          onPress={onExport}
        />

        <Text variant="h3" style={styles.heading}>{t('caccount.confirmItTitle')}</Text>
        {isPasswordAccount ? (
          <Input
            label={t('caccount.passwordLabel')}
            placeholder={t('caccount.passwordPlaceholder')}
            value={password}
          ...
          <Text variant="bodySm" tone="textMuted" style={styles.para}>
            {t('caccount.reauthUnavailable')}
          </Text>
        )}

        <Input
          label={t('caccount.typeEmailLabel')}
          placeholder={email}
          value={typed}
