...
import { useConsumerOrder } from '../../../../lib/consumer-queries';
...
export default function ConsumerOrderDetail() {
  ...
  if (isLoading || !data) {
    return (
      <Screen>
        <ScreenHeader title={t('corders.detailTitle')} />
        <SkeletonDetail />
      </Screen>
    );
  }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 96 }}>
        ...
        <View style={[styles.hero, { backgroundColor: colors.surface, borderColor: colors.hairline }]}>
          <View style={[styles.chip, { backgroundColor: withAlpha(tone, 0.16), alignSelf: 'flex-start' }]}>
            <Text variant="caption" style={{ color: tone }}>{t(`corders.status_${order.status}`)}</Text>
          </View>
        ...
        {/* Photos */}
        {photos.length > 0 ? (
          <Section title={t('corders.photosSection')} colors={colors}>
        ...
        {/* Pieces */}
        {items.length > 0 ? (
          <Section title={t('corders.itemsSection')} colors={colors}>
            {items.map((it) => (
              <View key={it.id} style={[styles.itemRow, { borderTopColor: colors.hairline }]}>
                ...
                <Text variant="bodySm" tone="textMuted">{t('corders.quantity', { count: it.quantity })}</Text>
              </View>
            ))}
          </Section>
        ) : null}
        ...
        {statusChanges.length > 0 ? (
          <Section title={t('corders.timelineSection')} colors={colors}>
            {statusChanges.map((e) => (
              <View key={e.id} style={styles.timelineRow}>
                <View style={[styles.dot, { backgroundColor: colors.primary }]} />
                <View style={{ flex: 1 }}>
                  <Text variant="bodySm">
                    {t('corders.updatedTo', { status: t(`corders.status_${e.toStatus as OrderStatus}`) })}
                  </Text>
